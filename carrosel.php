<!--Carrosel-->
<div id="carouselExampleCaptions" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-indicators">
          <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="0" class="active" aria-current="true" aria-label="Slide 1"></button>
          <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="1" aria-label="Slide 2"></button>
          <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="2" aria-label="Slide 3"></button>
        </div>
        <div class="carousel-inner">
          <div class="carousel-item active " data-interval = 5000>
              <div id="imagemtxt1" class="carousel-caption d-none d-md-block rounded-5 shadow-4-strong">
              <h1>Bem-vindo ao SenacLib</h1>
              <h2> Biblioteca acadêmica virtual</h2>
            </div>
          </div>
 
          <div class="carousel-item " data-interval = 5000>
            <div id="imagemtxt2" class="carousel-caption d-none d-md-block rounded-5 shadow-4-strong">
              <h1>Diversos conteúdos relacionados a Língua Portuguesa</h1>
              <h2>Selecionados para diversas normas gramáticais</h2>
            </div>
          </div>
 
          <div class="carousel-item " data-interval = 5000>
            <div id="imagemtxt3" class="carousel-caption d-none d-md-block rounded-5 shadow-4-strong" >
              <h1 style="text-size-adjust: 100px;">Esperamos que goste</h1>
              <h2>Visite a página "Sobre" para nos conhecer melhor </h2>
            </div>
          </div>
        </div>
      </div>
      </div>
 <!--Carrosel-->